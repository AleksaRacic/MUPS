...
int main(int arc, char **argv)
{
  ...
  int chunk_size = (ni + size - 1) / size; // gornji ceo deo
  int start = rank * chunk_size + 1;
  int end = start + chunk_size > ni ? ni : start + chunk_size;
  ...
  MPI_Reduce(&err, &master_err, 1, MPI_DOUBLE, MPI_SUM, MASTER, MPI_COMM_WORLD);
  MPI_Reduce(&n_inside, &master_n_inside, 1, MPI_INT, MPI_SUM, MASTER, MPI_COMM_WORLD);

  if (rank == MASTER) {
    master_err = sqrt(master_err / (double)(master_n_inside));
