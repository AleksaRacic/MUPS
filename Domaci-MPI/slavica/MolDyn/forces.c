...
void forces(int npart, double x[], double f[], double side, double rcoff)
{
  ...
  int chunk_size = (npart * 3 + size - 1) / size; // gornji ceo deo
  ...
  double f_master[npart*3];

  double vir_m, epot_m;
  MPI_Allreduce(f, f_master, npart * 3, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&vir, &vir_m, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&epot, &epot_m, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
