...
#pragma omp parallel for default(none) \
                          shared(sideh, side, npart,  rcoffs, f, x) \
                          private(i, j, xi, yi ,zi , xx, yy, zz, rrd, rd, rrd2, rrd3, \
                          rrd4, rrd6, rrd7, r148, forcex, forcey, forcez, fxi, fyi, fzi) \
                          reduction(+:vir, epot) \
                          schedule(static, 5)
  for (i = 0; i < npart * 3; i += 3)
  {
    ...
    for (j = i + 3; j < npart * 3; j += 3)
    {
      ...
      if (rd <= rcoffs)
      {
        ...
        forcex = xx * r148;
        fxi += forcex;
        //ne moze sa reduction pa sam stavio atomic
        #pragma omp atmoic
        f[j] -= forcex;
        forcey = yy * r148;
        fyi += forcey;
